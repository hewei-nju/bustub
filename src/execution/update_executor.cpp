  ...
  if (child_executor_->Next(tuple, rid)) {
    ...
    if (ret && !index_infos_.empty()) {
      for (auto & index_info : index_infos_) {
        Tuple src_key = tuple->KeyFromTuple(table_info_->schema_, index_info->key_schema_,
          index_info->index_->GetKeyAttrs());
        Tuple dest_key = dest_tuple.KeyFromTuple(table_info_->schema_, index_info->key_schema_,
          index_info->index_->GetKeyAttrs());
